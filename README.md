...
| `cursor` | `~/.cursor/rules/agent-bridge.mdc` | `~/.cursor/skills/` | `~/.cursor/mcp.json` → `mcpServers` |
...
- Cursor IDE Settings → User Rules is not a stable file API; global instructions are written to `~/.cursor/rules/agent-bridge.mdc` with `alwaysApply: true`.
